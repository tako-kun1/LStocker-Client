...
- 右上の「PDF出力」アイコンで在庫一覧をPDF保存できます
- 右上の「印刷」アイコンでローカルプリンターへ直接印刷できます
- ヘッダには `印刷日時` を `yyyy/MM/dd HH:mm` 形式で表示します
- 印刷テーブルの列は `登録日時 / 商品名 / JANコード / 期限日 / 在庫数 / 概要` です
- `概要` 列はメモを書き込めるよう、空欄かつ広めの枠で出力されます
- 出力対象は現在表示中の未アーカイブ在庫で、項目は商品名・JAN・数量・登録日・期限日です

### 通知

- 販売制限開始日が近い在庫を確認できます
- Androidではローカル通知も利用します
- ホーム画面の「通知」メニューに未読通知ドットを表示します
- 通知画面を開くと既読化され、ドットは非表示になります
...
- 起動時の自動確認を切り替えられます
- 手動でアップデート確認を実行できます
- キャッシュ済み更新情報は現在アプリ版で再評価し、更新済み端末での無限更新ループを防止しています
...
- 「DBデータ削除して最新CSVを取得する」ボタンで商品・在庫・同期キューを全削除してから最新CSVを再取得できます（確認ダイアログあり）

#### お問い合わせ

- お問い合わせ先リンクをタップするとURLのQRコードを表示します
- QRコード画面下の「ブラウザで開く」ボタンからURLを直接開けます
